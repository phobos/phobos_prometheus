RSpec.describe PhobosPrometheus::Collector do
  ...
  describe 'consumer events' do
    ...
    it 'tracks total events' do
      expect(subject.listener_events_total.values).to match({
        {:topic=>"topic_1", :group_id=>"group_1", :handler=>"AppHandlerOne"}=>1.0,
    ...
    it 'track total duration' do
      expect(subject.listener_events_duration.values).to match({
